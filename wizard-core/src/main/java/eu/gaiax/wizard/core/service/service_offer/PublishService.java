...
public class PublishService {
    ...
    public void publishServiceComplianceToMessagingQueue(UUID serviceOfferId, String complianceCredential) throws JsonProcessingException {
        ...
        publishToQueueRequest.setSource(wizardHost);
        publishToQueueRequest.setData((Map<String, Object>) objectMapper.readValue(complianceCredential, Map.class).get("complianceCredential"));

        try {
            ResponseEntity<Object> publishServiceComplianceResponse = messagingQueueClient.publishServiceCompliance(publishToQueueRequest);
            if (publishServiceComplianceResponse.getStatusCode().equals(HttpStatus.CREATED)) {
                if (publishServiceComplianceResponse.getHeaders().containsKey("location")) {
                    ...
                    serviceOfferRepository.updateMessageReferenceId(serviceOfferId, messageReferenceId);
